...
import { GraduationCap, Rocket, Users, StickyNote, Lightbulb, Code, User, Star } from "lucide-react"; // Menambahkan icon Star

const AboutPage = () => {
  const [isInViewHimatifta, setIsInViewHimatifta] = useState(false);
  const [isInViewAboutMe, setIsInViewAboutMe] = useState(false); // State baru untuk About Me

  useEffect(() => {
    // Menggunakan Intersection Observer untuk HIMATIFTA
    ...
    if (targetHimatifta) {
      observerHimatifta.observe(targetHimatifta);
    }

    // Menggunakan Intersection Observer untuk About Me
    ...
    if (targetAboutMe) {
      observerAboutMe.observe(targetAboutMe);
    }

    // Bersihkan observer setelah komponen di-unmount
    return () => {
      if (targetHimatifta) {
        observerHimatifta.unobserve(targetHimatifta);
      }
      if (targetAboutMe) {
        observerAboutMe.unobserve(targetAboutMe);
      }
    };
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#1d2533] text-white font-sans">
      {/* Section: Tentang NotesApp */}
      <section className="py-16 px-6 md:px-20">
        <div className="grid md:grid-cols-2 gap-10 items-center">
          <img
            src="/images/notesapp.jpeg"
            alt="NotesApp Preview"
            className="rounded-lg w-full object-cover shadow-2xl border border-blue-500 animate-slideUp"
          />
      ...
      <section
        ...
        className={`py-16 px-6 md:px-20 bg-[#f1f5f9] text-[#1d2533] ${isInViewHimatifta ? "animate-slideUp" : "opacity-0"}`}
      >
        <div className="grid md:grid-cols-2 gap-10 items-center">
          <img
            src="/images/himatifta.png"
            alt="HIMATIFTA"
            className="rounded-lg w-full object-cover shadow-lg border-none"
          />
          <div className={`transition-opacity duration-700 ease-in-out ${isInViewHimatifta ? "opacity-100" : "opacity-0"}`}>
            <h2 className="text-3xl md:text-4xl font-bold mb-4 uppercase flex items-center gap-2">
      ...
      {/* Section: About Me / My Profile */}
      <section
        id="about-me-section"
        className={`py-16 px-6 md:px-20 bg-[#283141] text-white ${isInViewAboutMe ? "animate-slideUp" : "opacity-0"}`}
      >
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-extrabold tracking-wide uppercase">About Me</h2>
        </div>
